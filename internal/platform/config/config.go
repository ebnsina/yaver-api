...
// atoi parses a non-negative int env value, defaulting to 0 when empty/invalid.
...
type Config struct {
	...
	STTProvider       string // "noop" (dev) | "whisper" — call transcription
	WhisperURL        string // required when STTProvider == "whisper"
	WhisperKey        string
	WhisperModel      string
	CallRetentionDays int    // prune calls older than N days (0 = keep forever)
...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		STTProvider:       req("YAVER_STT_PROVIDER"),
		WhisperURL:        os.Getenv("YAVER_WHISPER_URL"),
		WhisperKey:        os.Getenv("YAVER_WHISPER_API_KEY"),
		WhisperModel:      os.Getenv("YAVER_WHISPER_MODEL"),
		CallRetentionDays: atoi(os.Getenv("YAVER_CALL_RETENTION_DAYS")),
	...
	if cfg.STTProvider == "whisper" && cfg.WhisperURL == "" {
		return Config{}, fmt.Errorf("YAVER_WHISPER_URL is required when YAVER_STT_PROVIDER=whisper")
	}
